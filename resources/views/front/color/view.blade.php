...
@section('content')
    <main>
        <div class="container-fluid px-4">
            <h2 class="mt-4 main_heading">Colors</h2>
            <hr class="w-50 mx-auto">
             ...
             @if (in_array('lookup_create', roles_and_permissions()) ||
                ...
                <div class="d-flex justify-content-between mb-3">
                    <div>
                        {{-- <a class="btn btn-danger text-white" href="{{ route('color.trash') }}">Trash</a> --}}
                         @if (in_array('lookup_archive', roles_and_permissions()))
                        <a class="btn btn-success text-white" href="{{ route('color.archive') }}">Archive</a>
                        @endif
                    </div>
                     @if (in_array('lookup_create', roles_and_permissions()))
                    <a class="btn btn-info text-white" href="{{ route('color.form') }}">Add Color</a>
                    @endif
                </div>
            @endif
            <div class="card mb-4">
                <div class="card-body">
                    {{-- @if (Auth::user()->role_id == 5)
                        <form action="{{ route('color.bulk_delete') }}" method="GET">
                    ...
                    <table id="datatablesSimple">
                        ...
                        <tbody>
                            ...
                            @foreach ($colors as $color)
                                <tr>
                                    <td><input type="checkbox" name="bulk_action[]" value="{{ $color->id }}"
                                            id=""></td>
                                    <td>{{ $key }}</td>
                                    <td>{{ $color->Name ?? '' }}</td>
                                    ...
                                    {{-- <td>{{ $color->CreatedBy == 1 ?'Public':'Private' }}</td> --}}
                                    @if (in_array('lookup_archive', roles_and_permissions()) ||
                                        in_array('lookup_update', roles_and_permissions()))
                                        <td>
                                            <div class="d-flex">
