...
@section('content')
    <main>
        <div class="container-fluid px-4">
            <h2 class="mt-4 main_heading">Assessments Evalute</h2>
        ...
        <div class="d-flex justify-content-between mb-3">
                    <div>
                        {{-- <a class="btn btn-danger text-white" href="{{ route('assessment_evaluate.trash') }}">Trash</a> --}}
                        @if (in_array('lookup_archive', roles_and_permissions()))
                        <a class="btn btn-success text-white" href="{{ route('assessment_evaluate.archive') }}">Archive</a>
            @endif
        </div>
                        @if (in_array('lookup_create', roles_and_permissions()))
                        <a class="btn btn-info text-white" href="{{ route('assessment_evaluate.form') }}">Add Assessment
                        Evalute</a>
            @endif
        </div>
            @endif
            <div class="card mb-4">
                <div class="card-body">
                    {{-- @if (Auth::user()->role_id == 5)
                        <form action="{{ route('assessment_evaluate.bulk_delete') }}" method="GET">
                                ...
                                <tr>
                                    ...
                                    <td>
                                            <div class="d-flex">
                                                ...
                                                <a href="{{ route('assessment_evaluate.edit', ['id' => $assessment->id]) }}"
                                                    class="btn btn-info text-white">
                                                    <i class="fa fa-pen"></i>
                                                </a>
                                                @endif
                                                {{-- <a href="{{ route('assessment_evaluate.delete', ['id' => $assessment->id]) }}"
                                                    class="btn btn-danger me-2">
                                                    <i class="fa fa-trash"></i>
                                                </a> --}}
                                                @if (in_array('lookup_archive', roles_and_permissions()))
                                                <a href="{{ route('assessment_evaluate.toggle.archive', ['id' => $assessment->id]) }}"
