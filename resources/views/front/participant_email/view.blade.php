...
@section('content')
    <main>
        <div class="container-fluid px-4">
            <h2 class="mt-4 main_heading">Participant Emails</h2>
            ...
            in_array('participant_delete', roles_and_permissions()))
               <div class="d-flex justify-content-between mb-3">
                    <div>
                       ...
                       <a class="btn btn-danger text-white" href="{{ route('participant_email.trash') }}">Trash</a>
                    @endif
                    @if (in_array('participant_archive', roles_and_permissions()))
                    <a class="btn btn-success text-white" href="{{ route('participant_email.archive') }}">Archive</a>
                    @endif
                </div>
                    @if (in_array('participant_create', roles_and_permissions()))
                        <a class="btn btn-info text-white" href="{{ route('participant_email.form') }}">Add Participant
                            Email</a>
                    @endif
                </div>
            @endif
            <div class="card mb-4">
                <div class="card-body">
                    @if (in_array('participant_delete', roles_and_permissions()))
                        <form action="{{ route('participant_email.bulk_delete') }}" method="GET">
                    ...
                    <table id="datatablesSimple">
                        ...
                        <tbody>
                            ...
                            @foreach ($participant_emails as $participant_email)
                                <tr>
                                    <td><input type="checkbox" name="bulk_action[]" value="{{ $participant_email->id }}"
                                            id=""></td>
                                    <td>{{ $key }}</td>
                                    ...
                                    in_array('participant_delete', roles_and_permissions()))
                                        <td>
                                            <div class="d-flex">
                                                @if (in_array('participant_update', roles_and_permissions()))
                                                    <a href="{{ route('participant_email.edit', ['id' => $participant_email->id]) }}"
                                                        class="btn btn-info text-white">
                                                        <i class="fa fa-pen"></i>
                                                    </a>
                                                @endif
                                                @if (in_array('participant_delete', roles_and_permissions()))
                                                    <a href="{{ route('participant_email.delete', ['id' => $participant_email->id]) }}"
                                                        class="btn btn-danger me-2">
                                                        <i class="fa fa-trash"></i>
                                                    </a>
                                                @endif
                                                @if (in_array('participant_archive', roles_and_permissions()))
                                                <a href="{{ route('participant_email.toggle.archive', ['id' => $participant_email->id]) }}"
