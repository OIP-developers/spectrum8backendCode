...
@section('content')
    <main>
        <div class="container-fluid px-4">
         ...
         @if (in_array('assessment_result_archive', roles_and_permissions()) ||
            in_array('assessment_result_delete', roles_and_permissions()))
                <div class="d-flex justify-content-between mb-3">
                    <div>
                        @if (in_array('assessment_result_delete', roles_and_permissions()))
                        <a class="btn btn-danger text-white" href="{{ route('result.trash') }}">Trash</a>
                        @endif
                        @if(in_array('assessment_result_archive', roles_and_permissions()))
                        <a class="btn btn-success text-white" href="{{ route('result.archive') }}">Archive</a>
                        @endif
                    </div>
                </div>
            @endif
                            ...
                            {{-- @if (isset($results_data))
                                @foreach ($results_data as $result)
                                    <tr>
                                        ...
                                        <td>{{ $result->first()->resultGender->first()->Gender ?? '' }}
                                        </td>
                                        <td>{{ $result->first()->Score }}</td>
                                        <td>{{ $result->sum('Score') }}</td>
                                        <td>
                                            @if (!empty($result->first()->assessment_scoring_data->first()) && $result->first()->assessment_scoring_data->first()->image != null)
                            ...
                            @foreach ($results as $result)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span style="display:none;"> {{ $result->color->Name ?? '' }}</span>
                                    </td>
                                    <td>{{ $result->Results ?? '' }}
                                    <td>{{ $result->DTRecorded ?? '' }}
                                    <td>{{ $result->Attempt ?? '' }}
                                ...
                                in_array('assessment_result_delete', roles_and_permissions()))
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            @endif
                                            @if (in_array('assessment_result_archive', roles_and_permissions()))
                                            <a href="{{ route('result.toggle.archive', ['id' => $result->id]) }}"
