...
class CreateFacilitatorClassesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('facilitator_classes', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('FacilitatorID')->unsigned()->nullable(); 
            $table->integer('ClassID')->unsigned()->nullable(); 
            $table->string('ClassName',15)->nullable(); 
    ...
    {
        Schema::dropIfExists('facilitator_classes');
    }
}
